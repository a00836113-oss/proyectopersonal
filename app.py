...
@st.cache_data
def load_data():
    # CSV alojado en Google Drive
    url = "https://drive.google.com/uc?export=download&id=1L5s_WqmSVn8CrASlL9mZ18bZ7rEIFP1Q"
    df = pd.read_csv(url)
    # Aseguramos que la columna fecha quede en datetime (como antes)
...
# Mapeo de label numérico a texto
...
with tab2:
    ...
    with col1:
        subjects_time = st.multiselect(
            ...
            default=sorted(df_with_date["subject"].dropna().unique()[:10]),
            key="tab2_subjects"
        )
...
with tab5:
    ...
    cols_to_show = [c for c in ["date", "label_name", "subject", "title", "text_word_len"] if c in df.columns]

    st.dataframe(
        df[cols_to_show].head(200),
