...
# =========================
# CARGA DE DATOS DESDE GOOGLE DRIVE (ROBUSTA)
# =========================
@st.cache_data
def load_data():
    # Enlace directo a tu CSV en Google Drive
    url = "https://drive.google.com/file/d/1L5s_WqmSVn8CrASlL9mZ18bZ7rEIFP1Q/view?usp=sharing"
    ...
    if "date" in df.columns:
        ...
        # Buscar alguna columna que en limpio se llame "date"
        candidata = None
        ...
        if candidata is not None:
            df.rename(columns={candidata: "date"}, inplace=True)
            df["date"] = pd.to_datetime(df["date"], errors="coerce")
        else:
            # Si definitivamente no hay fecha, creamos una columna vacía
            df["date"] = pd.NaT
